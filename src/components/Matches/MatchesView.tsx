...
export const MatchesView: React.FC<MatchesViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'grid' | 'live_featured'>('grid');
  ...
  const findMatchingChannel = (channelName: string): ChannelItem | undefined => {
    ...
    const keywords = ['espn', 'dazn', 'fox', 'tnt', 'tudn', 'movistar', 'dsports', 'directv', 'sky', 'claro', 'vix'];
  ...
  const handleLaunchChannel = (matchChannel: MatchChannel, match: SportsMatch) => {
    ...
    // Try finding in user's playlist
    ...
    // If available in channels list (first sports channel or current list)
    const fallbackSports = channels.find((c) =>
  ...
  const featuredMatch = useMemo(() => {
    return matches.find((m) => m.status === 'live' && m.isHot) ||
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-zinc-950 overflow-y-auto p-3 sm:p-5 lg:p-6 space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-zinc-900 via-indigo-950/40 to-zinc-900 border border-zinc-800/80 rounded-3xl p-4 sm:p-5 shadow-xl">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              Guía de encuentros deportivos y canales con emisión en directo
            </p>
          </div>
        </div>

        {/* Action Buttons & Realtime Widget */}
        <div className="flex items-center gap-2.5 flex-wrap justify-end">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300 text-xs transition-colors cursor-pointer"
            title="Restablecer calendario oficial"
          >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={`w-2 h-2 rounded-full ${inPlaylist ? 'bg-emerald-400 shadow-sm shadow-emerald-400/50' : 'bg-indigo-400'}`} />
      ...
      {/* Navigation Filter Controls & Search */}
      <div className="space-y-3">
        {/* Sport Categories Horizontal Carousel */}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              type="text"
              placeholder="Buscar equipo, torneo o canal..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl pl-10 pr-4 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
        </div>
      </div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      {/* Delete button (for custom matches or management) */}
                      <button
