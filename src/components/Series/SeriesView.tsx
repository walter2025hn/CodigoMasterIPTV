...
export const SeriesView: React.FC<SeriesViewProps> = ({
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full gap-4 p-3 lg:p-5 overflow-hidden">
      ...
      {/* Series Grid */}
      <div className="flex-1 bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-4 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Clapperboard className="w-5 h-5 text-purple-400" />
            <h3 className="text-sm font-bold text-white uppercase tracking-wider">
              Catálogo de Series ({filteredSeries.length})
            </h3>
          </div>
        </div>
        ...
        {filteredSeries.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-zinc-500">
            <Clapperboard className="w-12 h-12 mb-3 stroke-1 text-zinc-600" />
            <p className="text-base font-semibold text-zinc-300">No hay series en esta categoría</p>
            <p className="text-xs text-zinc-500 mt-1">Prueba seleccionando otra categoría o limpiando la búsqueda</p>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto pr-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3.5">
